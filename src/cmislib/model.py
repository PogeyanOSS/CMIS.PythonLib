...
class CmisClient(object):
    ...
    def __init__(self, repositoryUrl, username, password, **kwargs):
        ...
        self.extArgs = kwargs
        if 'binding' in kwargs.keys() and (isinstance(kwargs['binding'], Binding)):
    ...
    def fromString(self,sessionString):
          
          sessionJson =  json.loads(sessionString)
          self.repositoryUrl = sessionJson['url']
          self.username = sessionJson['username']
          self.password = sessionJson['password']
          return self,1
      
      
        
    def toString(self,sessionJson):
      
       sessionString = json.dumps(sessionJson)
