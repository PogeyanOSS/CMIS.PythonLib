...
from cmislib.domain import CmisId, CmisObject, ObjectType, ACL, ChangeEntry, ACE
from cmislib.exceptions import ObjectNotFoundException, NotSupportedException, CmisException, InvalidArgumentException
from cmislib.util import parsePropValueByType, safe_urlencode, safe_quote, parseDateTimeValue
...
class BrowserBinding(Binding):
    ...
    def post(self, url, payload, contentType, username, password, **kwargs):
        ...
        if resp.status_code != 200 and resp.status_code != 201:
            ...
            #print(resp.content)
            #if resp.content.decode('utf-8')!='' and resp.content.decode('utf-8')!='{}' :
            result = json.loads(resp.content.decode("utf8"))
            # else:
                 # result={}
...
class BrowserCmisObject(object):
    ...
    def resetCache(self,**kwargs):
        ...
        repoUrl = self._repository.getRepositoryUrl()+'/cache'  + "?&cmisselector=resetcache"
        result = self._cmisClient.binding.get(repoUrl.encode('utf-8'),
                                          self._cmisClient.username,
                                          self._cmisClient.password,
                                          **kwargs)
        return result
    
    def resetCacheByKey(self,key,**kwargs): 
      
      props={"key":key}
      repoUrl = self._repository.getRepositoryUrl() + '/cache' + "?&cmisselector=resetcachebykey&"+key
      result = self._cmisClient.binding.get(repoUrl.encode('utf-8'),
                                          self._cmisClient.username,
                                          self._cmisClient.password,
                                          **kwargs)
      ...
      return result
    def addPropertiesIds(self, options, inputIds):
        i = 0
        options['propertyId[' + str(i) + ']'] = "ids"
        for j in range(len(inputIds)):
            options['propertyValue[' + str(i) + '][' + str(j) + ']'] = inputIds[j]
  
      
    def getAllObjects(self,ids):
        repoUrl = self._repository.getRootFolderUrl()
        props = {"cmisaction": "getAllObjects"}
        self.addPropertiesIds(props,ids)
        ...
        result = self._cmisClient.binding.post(repoUrl.encode('utf-8'),
                                               ...
                                               self._cmisClient.password)
        return result
    def setProperties(self,options,properties):
        i=0
        for id in properties:
            options['propertyId[' + str(i) + ']'] = id
            propertyValue = properties[id]
            if bool(propertyValue):
                if type(propertyValue) is not str:
                    for j in range(len(propertyValue)):
                        options['propertyValue[' + str(i) + '][' + str(j) + ']'] = propertyValue[j]
                else:
                    options['propertyValue[' + str(i) + ']'] = propertyValue
            i+=1

    def setACEs(self,options,ACEs,action):
        """ format ACEs for request""" 
        if action!='add' and action!='remove':
            #exception
            return 
        i=0    
        for id in ACEs:
            options[action + 'ACEPrincipal[' + str(i) + ']'] = id
            ace = ACEs[id]
            for j in range(len(ace)):
                options[action + 'ACEPermission[' + str(i) + '][' + str(j) + ']'] = ACEs[id][j]
            i+=1
            
            
    def setPolicies(self,options,policies):
        """format policies for requests"""
        if bool(policies):
            for i in range(len(policies)):
                options['policy[' + str(i) + ']'] = policies[i]
                
    def bulkInsert(self,properties,**kwargs):
        """bulkInsert"""
        ...
        if "createItem" in properties and bool(properties["createItem"]):
            for itemInput in properties["createItem"]:
                dop = {}
                if 'addAces' in itemInput:
                    addAces = itemInput["addAces"]
                    self.setACEs(dop, addAces, "add")
                    del itemInput["addAces"]
                
                if 'removeAces' in itemInput:
                    removeAces = itemInput['removeAces']
                    self.setACEs(dop, removeAces, "remove")
                    del itemInput["removeAces"]
                '''    
                if 'policies' in itemInput:
                    policies = itemInput['policies']
                    self.setPolicies(dop, policies)
                    del itemInput["policies"]'''
                
                setProps(itemInput,dop)
                createIteList.append(dop)
        
            del properties["createItem"]
    
        if 'createDocument' in properties and bool(properties['createDocument']):
            for docInput in properties['createDocument']:
                dop={}
                if 'cmis:objectId' not in docInput or not bool(docInput['cmis:objectId']):
                    docInput['cmis:objectId']=uuid.uuid1()
                
                if 'addAces' in docInput:    
                    addAces = docInput['addAces']
                    self.setACEs(dop, addAces, "add")
                    del docInput["addAces"]
                
                if 'removeAces' in docInput:
                    removeAces = docInput['removeAces']
                    self.setACEs(dop, removeAces, "remove")
                    del docInput["removeAces"]
                '''
                policies = docInput['policies']
                self.setPolicies(dop, policies)
                del docInput["policies"]'''
                
                #Might require changes
                if 'content' in docInput and bool(docInput["content"]):
                    multipartData = docInput["content"]
                    if 'filename' not in multipartData or not bool(multipartData['filename']):
                        multipartData['filename']=docInput['cmis:name']
                    multipartDataList.append({
                                                "multipartData": multipartData,
                                                "name": str(docInput['cmis:objectId'])+'_'+docInput['cmis:name'] })
                    del docInput['content']
                
                setProps(docInput,dop)
                createDocList.append(dop)
            del properties['createDocument']
            
        if 'createFolder' in properties and bool(properties['createFolder']):
            for folInput in properties['createFolder']:
                dop={}
                if 'addAces' in folInput:
                    addAces = folInput["addAces"]
                    self.setACEs(dop, addAces, "add")
                    del folInput["addAces"]
                
                if 'removeAces' in folInput:
                    removeAces = folInput["removeAces"]
                    self.setACEs(dop, removeAces, "remove")
                    del folInput["removeAces"]
                '''    
                policies = folInput["policies"]
                self.setPolicies(dop, policies)
                del folInput["policies"]'''
                
                setProps(folInput,dop)
                createFolList.append(dop)
            del properties["createFolder"]
            
        if 'createPolicy' in properties and bool(properties['createPolicy']):
            for polInput in properties['createPolicy']:
                dop={}
                if 'addAces' in polInput:
                    addAces = polInput["addAces"]
                    self.setACEs(dop, addAces, "add")
                    del polInput["addAces"]
                
                if 'removeAces' in polInput:
                    removeAces = polInput["removeAces"]
                    self.setACEs(dop, removeAces, "remove")
                    del polInput["removeAces"]
                '''    
                policies = polInput["policies"]
                self.setPolicies(dop, policies)
                del polInput["policies"]'''
                
                setProps(polInput,dop)
                createPolList.append(dop)
            del properties["createPolicy"]
            
        if 'createRelationship' in properties and bool(properties['createRelationship']):
            for relInput in properties['createRelationship']:
                dop = {}
                if 'addAces' in relInput:
                    addAces = relInput["addAces"]
                    self.setACEs(dop, addAces, "add")
                    del relInput["addAces"]
                
                if 'removeAces' in relInput:
                    removeAces = relInput["removeAces"]
                    self.setACEs(dop, removeAces, "remove")
                    del relInput["removeAces"]
                '''    
                policies = relInput["policies"]
                self.setPolicies(dop, policies)
                del relInput["policies"]'''
                
                setProps(relInput,dop)
                createRelList.append(dop)
                
            del properties["createRelationship"]
        ...
        contentType, body=encode_multipartDataList_formdata(properties,multipartDataList,None)
    ...
    def bulkUpdate(self, properties, **kwargs):
        properties["cmisaction"] = 'bulkupdateprops'
        updateList = []
        multipartDataList = []
        
        if 'update' in properties and bool(properties['update']):
            for updateInput in properties['update']:
                dop = {}
                
                if 'content' in updateInput and bool(updateInput['content']):
                    multipartData = updateInput["content"]
                    if "filename" not in multipartData or bool(multipartData["filename"]):
                        multipartData["filename"] = updateInput["cmis:objectId"]
                    multipartDataList.append({"multipartData": multipartData,
                                              "name": updateInput["cmis:objectId"]})
                    del updateInput['content']
                    
                setProps(updateInput, dop)
                updateList.append(dop)
                
            del properties["update"]
    ...
    def bulkDelete(self,ids,options={}):
        options['cmisaction']='bulkdelete'
...
class BrowserRepository(object):
    ...
    def getCapabilities(self):
        ...
        Changes:None
        """
        #print('inside capabilities')
        if not self._capabilities:
            #print(self.data['capabilities'])
            if self.data is None:
                self.reload()
            caps = {}
    ...
    def getContentChanges(self, **kwargs):
        ...
        #print()
        #print(self._cmisClient.username)
        #print(self._cmisClient.password)
...
class BrowserFolder(BrowserCmisObject):
    ...
    def createDocument(self, name, properties={}, contentFile=None,
                       contentType=None, contentEncoding=None, fileName=None):
...
def encode_multipartDataList_formdata(fields, multipartDataList, contentType):
    ...
    if multipartDataList and type(multipartDataList) is list:
            for data in multipartDataList:
                multipartData = data['multipartData']
                L.append('--' + boundary)
                if 'filename' in multipartData:
                    if '.' not in multipartData['filename'] and 'mimeTypeExtension' in multipartData:
                        fileName = multipartData['filename']+'.'+multipartData['mimeTypeExtension']
                    else:
                        fileName = multipartData['filename']
                L.append('Content-Disposition: form-data; name="%s"; filename=%s' % (data['name'], fileName))
                L.append('Content-Type: %s' % contentType)
                L.append('Content-Transfer-Encoding: binary')
                L.append('')
