...
class QueryTest(CmisTestBase):
    ...
    def setUp(self):
        ...
        testFile = open(settings.TEST_BINARY_2, 'r')
...
class RepositoryTest(CmisTestBase):
    ...
    def testCreateDocumentFromString(self):
        ...
        self.assertEqual(documentName, newDoc.getName())
        self.assertEqual((newDoc.getContentStream().read()).decode('utf-8'), contentString)

    # CMIS-279
    
    def testCreateDocumentUnicode(self):
        """Create a new doc with unicode characters in the name"""
        documentName = u'abc cde$$@@@!-_caf@@!.txt'
        newDoc = self._repo.createDocument(documentName, parentFolder=self._rootFolder)
        self.assertEqual(documentName, newDoc.getName())
    ...
    def testReturnVersion(self):
        ...
        f = open(settings.TEST_BINARY_1, 'r')
...
class FolderTest(CmisTestBase):
    ...
    def testResetCache(self):
        rootFolder = self._repo.getRootFolder()
        #print(rootFolder)
        result=rootFolder.resetCache()
        self.assertTrue(result is not None)
    
    def testResetCacheByKey(self):
        rootFolder = self._repo.getRootFolder()
        result=rootFolder.resetCacheByKey('cmis:folder')
        self.assertTrue(result is not None)
    
    def testGetAllObjects(self):
        
        resp = self._rootFolder.getAllObjects(['5efc138d5b1c3800123434b4'])
    ...
    def testBulkDelete(self):
        ...
        rootFolder = self._repo.getRootFolder()
        ids = ['Doc_26','Doc_27']
        rootFolder.bulkDelete(ids)
    
    #Each time range should be different for assertion to be proper
    #The order of item,fol,doc,pol in response is changed when repeated for same values
    def testBulkInsert(self):
        ...
        for i in range(110,115):
            input = { "cmis:objectId": "Item_" + str(i), "cmis:name": "Item_" + str(i), "cmis:objectTypeId": "cmis:item" }
            input["addAces"] = aces
            #input["policies"] = ["policy123"]
            itemList.append(input)
    
        itemList.append({ "source_table": "cmis:folder", "target_table": "cmis:document", "cmis:name": "folder_document", "cmis:objectTypeId": "cmis_ext:relationmd" })

        for i in range(110,115):
            input = { "cmis:objectId": "Doc_" + str(i), "cmis:name": "Doc_" + str(i), "cmis:objectTypeId": "cmis:document" }
            input["addAces"] = aces
            #input["policies"] = ["policy123"]
            docList.append(input)
            
        for i in range(110,115):
            input = { "cmis:objectId": "Fol_" + str(i), "cmis:name": "Fol_" + str(i), "cmis:objectTypeId": "cmis:folder" }
            input["addAces"] = aces
            #input["policies"] = ["policy123"]
            folList.append(input)
    
        for i in range(110,115):
            input = { "cmis:objectId": "Pol_" + str(i), "cmis:name": "Pol_" + str(i), "cmis:objectTypeId": "cmis:policy" }
            input["addAces"] = aces
            #input["policies"] = ["policy123"]
            polList.append(input)
    
        for i in range(110,115):
            input = { "relation_name": "folder_document", "cmis:name": "Rel_" + str(i), "cmis:objectTypeId": "cmis_ext:relationship", "cmis:sourceId": "Fol_" + str(i), "cmis:targetId": "Doc_" + str(i) }
            input["addAces"] = aces
            #input["policies"] = ["policy123"]
            relList.append(input)
        ...
        #call the function
        repo = self._repo.getRootFolder()
        #print(repo)
        res = json.loads(repo.bulkInsert(props))
        
        self.assertEqual(res["success"]["objects"][0]["object"]["succinctProperties"]["cmis:objectId"], "Fol_110")
        self.assertEqual(res["success"]["objects"][1]["object"]["succinctProperties"]["cmis:objectId"], "Fol_111")
        self.assertEqual(res["success"]["objects"][2]["object"]["succinctProperties"]["cmis:objectId"], "Fol_112")
        self.assertEqual(res["success"]["objects"][3]["object"]["succinctProperties"]["cmis:objectId"], "Fol_113")
        self.assertEqual(res["success"]["objects"][4]["object"]["succinctProperties"]["cmis:objectId"], "Fol_114")
        #print(res)
    
    def testBulkInsertDocs(self):
        props = {}
        docList = []

        aces = {}
        aces['UserA'] = ['cmis:all']
        for i in range(100,105):  #change range to create new test set files
            input = { "cmis:objectId": "Doc_" + str(i), "cmis:name": "Doc_" + str(i), "cmis:objectTypeId": "cmis:document" }
            input["addAces"] = aces
            input["content"] = {
                "content": 'default_' + str(i),
        ...
        docList.append({
               "content": {
                "content": 'default_107',
                "filename": "default_107",
                "mimeTypeExtension": 'txt'
              }, "cmis:objectId": "Doc_107", "cmis:name": "Doc_107", "cmis:objectTypeId": "cmis:document"
            })
        docList.append({
                "content": {
                "content": 'default_108',
                "filename": "default_108.txt"
              }, "cmis:objectId": "Doc_108", "cmis:name": "Doc_108", "cmis:objectTypeId": "cmis:document"
            })
        docList.append({
                "content": {
                "content": 'default_109',
                "filename": "default_109.txt",
                "mimeTypeExtension": 'txt'
              }, "cmis:objectId": "Doc_109", "cmis:name": "Doc_109", "cmis:objectTypeId": "cmis:document"
            })
        ...
        self.assertEqual(res["success"]["objects"][0]["object"]["succinctProperties"]["cmis:name"],"Doc_100")
        self.assertEqual(res["success"]["objects"][1]["object"]["succinctProperties"]["cmis:name"],"Doc_101")
        self.assertEqual(res["success"]["objects"][2]["object"]["succinctProperties"]["cmis:name"],"Doc_102")
        self.assertEqual(res["success"]["objects"][3]["object"]["succinctProperties"]["cmis:name"],"Doc_103")
        self.assertEqual(res["success"]["objects"][4]["object"]["succinctProperties"]["cmis:name"],"Doc_104")
        self.assertEqual(res["success"]["objects"][5]["object"]["succinctProperties"]["cmis:name"],"Doc_107")
        self.assertEqual(res["success"]["objects"][6]["object"]["succinctProperties"]["cmis:name"],"Doc_108")
        self.assertEqual(res["success"]["objects"][7]["object"]["succinctProperties"]["cmis:name"],"Doc_109")
    ...
    def testBulkUpdate(self):
        props = {}
        updateList = []
        updateList.append({ "cmis:objectId": "Item_114", "cmis:name": "Item_Updated", "cmis:objectTypeId": "cmis:item" });
        updateList.append({ "cmis:objectId": "Doc_114", "cmis:name": "Doc_Updated", "cmis:objectTypeId": "cmis:document"});
        updateList.append({ "cmis:objectId": "Fol_114", "cmis:name": "Fol_Updated", "cmis:objectTypeId": "cmis:folder" });
        updateList.append({ "cmis:objectId": "Pol_114", "cmis:name": "Pol_Updated", "cmis:objectTypeId": "cmis:policy" });

        props["update"] = updateList;
        repo = self._repo.getRootFolder()
        resp = json.loads(repo.bulkUpdate(props))
        #print(resp)
        self.assertEqual(resp["success"]["objects"][0]["object"]["succinctProperties"]["cmis:name"],"Item_Updated")
        self.assertEqual(resp["success"]["objects"][1]["object"]["succinctProperties"]["cmis:name"],"Doc_Updated")
        self.assertEqual(resp["success"]["objects"][2]["object"]["succinctProperties"]["cmis:name"],"Fol_Updated")
        self.assertEqual(resp["success"]["objects"][3]["object"]["succinctProperties"]["cmis:name"],"Pol_Updated")
        
   
    def testBulkUpdateDocs(self):
         
        props = {}
        updateList = []
        updateList.append({"content": {
                           "content": 'Doc_100_upload',
                           "filename": "Doc_100_upload.txt"
                           }, "cmis:objectId": "Doc_100", "cmis:name": "Doc_100_upload", "cmis:objectTypeId": "cmis:document" })
        updateList.append({"content": {
                           "content": 'Doc_101_upload',
                           "filename": "Doc_101_upload.txt"
                           }, "cmis:objectId": "Doc_101", "cmis:name": "Doc_101_upload", "cmis:objectTypeId": "cmis:document" })
        updateList.append({"content": {
                           "content": 'Doc_102_upload',
                           "filename": "Doc_102_upload.txt"
                           }, "cmis:objectId": "Doc_102", "cmis:name": "Doc_102_upload", "cmis:objectTypeId": "cmis:document" })
        updateList.append({"content": {
                           "content": 'Doc_103_upload',
                           "filename": "Doc_103_upload.txt"
                           }, "cmis:objectId": "Doc_103", "cmis:name": "Doc_103_upload", "cmis:objectTypeId": "cmis:document" })
        
        props["update"] = updateList
        repo = self._repo.getRootFolder()
        res = json.loads(repo.bulkUpdate(props))
        
        self.assertEqual(res["success"]["objects"][0]["object"]["succinctProperties"]["cmis:name"],"Doc_100_upload")
        self.assertEqual(res["success"]["objects"][1]["object"]["succinctProperties"]["cmis:name"],"Doc_101_upload")
        self.assertEqual(res["success"]["objects"][2]["object"]["succinctProperties"]["cmis:name"],"Doc_102_upload")
        self.assertEqual(res["success"]["objects"][3]["object"]["succinctProperties"]["cmis:name"],"Doc_103_upload")
...
class ChangeEntryTest(CmisTestBase):
    ...
    def testGetContentChanges(self):
        ...
        # test documents
        rs = self._repo.getContentChanges(changeLogToken=self._repo.info['latestChangeLogToken'])
...
class DocumentTest(CmisTestBase):
    ...
    def testCheckinContentAndProperties(self):
        ...
        testFilename = settings.TEST_BINARY_1.split('/')[-1]
        #print(testFilename)
        contentFile = open(testFilename, 'r',encoding='utf-8')
        props = {'cmis:objectTypeId': settings.VERSIONABLE_TYPE_ID}
        testDoc = self._testFolder.createDocument(testFilename, contentFile=contentFile, properties=props)
        contentFile.close()
        self.assertEqual(testFilename, testDoc.getName())
        ...
        try:
            ...
            contentFile2 = open(testFile2, 'r',encoding='utf-8')
    ...
    def testCancelCheckout(self):
        ...
        try:
            ...
            pwcDoc.delete()
        self.assertTrue(newDoc.isCheckedOut())   #True
    ...
    def testDeleteContentStreamPWC(self):
        ...
        # create a test document
        contentFile = open(settings.TEST_BINARY_1, 'r')
        props = {'cmis:objectTypeId': settings.VERSIONABLE_TYPE_ID}
        fileName = settings.TEST_BINARY_1.split('/')[-1]
        newDoc = self._testFolder.createDocument(fileName, contentFile=contentFile, properties=props)
        contentFile.close()
    ...
    def testCreateDocumentPlain(self):
        ...
        testFile.write(b'This is a sample text file line 1.')
        testFile.write(b'This is a sample text file line 2.')
        testFile.write(b'This is a sample text file line 3.')
        testFile.close()
        contentFile = open(testFilename, 'r')
        newDoc = self._testFolder.createDocument(testFilename, contentFile=contentFile)
        contentFile.close()
    ...
    def testGetAllVersions(self):
        ...
        #self.assertEqual('3.0', doc.getProperties()['cmis:versionLabel'])
...
#                             rs.getResults().values()[count].getProperties()['cmis:isLatestVersion'])
    ...
    def testGetObjectParents(self):
        ...
        childFolder = self._rootFolder.createFolder('parentTest')
        #print(childFolder)
        parentFolder = childFolder.getObjectParents().getResults()[0]
        #print(parentFolder)
        self.assertEqual(self._rootFolder.getObjectId(), parentFolder.getObjectId())
    ...
    def testRenditions(self):
        ...
        if not 'Renditions' in self._repo.getCapabilities():
            print('Repo does not support unfiling, skipping')
            return
...
def isInResultSet(resultSet, targetDoc):
    ...
    while not done:
        if targetDoc.getObjectId() in resultSet:
            return True
